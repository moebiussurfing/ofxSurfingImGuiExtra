...
using namespace ofxSurfingImGui;

void ofxSurfingImGui::Separator()
{
...
float ofxSurfingImGui::GetWidgetTitleDescriptionHeight(const char *title,
...
void ofxSurfingImGui::WidgetTitleDescription(const char *title, const char *description,
                            ImVec2 pos)
{
    ImDrawList *draw_list = ImGui::GetWindowDrawList();
    ImGuiStyle &style = ImGui::GetStyle();
...
void ofxSurfingImGui::WidgetTitleDescriptionItem(const char *str_id, const char *description)
{
...
float ofxSurfingImGui::GetSliderRadius(ImVec2 size)
{
    return size.y * 0.5;
}

float ofxSurfingImGui::GetSliderTrackXOffset(ImVec2 size)
{
    return GetSliderRadius(size);
}

float ofxSurfingImGui::GetSliderTrackWidth(ImVec2 size)
{
    return size.x - GetSliderRadius(size) * 2;
}

float ofxSurfingImGui::GetSliderValueForMousePos(ImVec2 mouse, ImVec2 pos, ImVec2 size)
{
    return (mouse.x - pos.x - GetSliderTrackXOffset(size)) /
           GetSliderTrackWidth(size);
}

void ofxSurfingImGui::DrawSlider(float v, bool hovered, ImVec2 pos, ImVec2 size)
{
    ImDrawList *draw_list = ImGui::GetWindowDrawList();

    float radius = GetSliderRadius(size);
    float rounding = size.y * 0.75;
...
void ofxSurfingImGui::DrawToggle(bool enabled, bool hovered, ImVec2 pos, ImVec2 size)
{
    ImDrawList *draw_list = ImGui::GetWindowDrawList();
...
bool ofxSurfingImGui::Toggle(const char *str_id, bool *v, const char *description)
{
...
void ofxSurfingImGui::Slider(const char *str_id, float *v, const char *description)
{
    ImGui::PushStyleColor(ImGuiCol_Button, IM_COL32_BLACK_TRANS);
